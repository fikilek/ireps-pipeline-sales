...
$BackupPath = "$RulesPath.before-governance-$Timestamp.bak"
...
$updated = Get-Content $RulesPath -Raw -Encoding UTF8
$required = @(
    '**Version:** 1.7',
    'DATA AND FIRESTORE BASELINE COMPLETE',
    'sales-all-meters                 35,295 documents',
...
foreach ($item in $required) {
    if (-not $updated.Contains($item)) {
        throw "Rules validation failed. Missing: $item"
    }
}

Write-Host ""
Write-Host "=== SALES PIPELINE RULES UPDATE COMPLETE ==="
Write-Host "Rules updated: $RulesPath"
Write-Host "Backup:        $BackupPath"
Write-Host "Validation:    PASS"
Write-Host ""
Write-Host "No schema file, Firebase collection, CSV or credential was changed."
