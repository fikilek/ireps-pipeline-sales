...
def main() -> int:
    ...
    try:
        if args.read_batch_size != DEFAULT_READ_BATCH_SIZE:
            raise ValueError(
                f"--read-batch-size is governed and must equal {DEFAULT_READ_BATCH_SIZE}"
            )
        if args.write_batch_size != DEFAULT_WRITE_BATCH_SIZE:
            raise ValueError(
                f"--write-batch-size is governed and must equal {DEFAULT_WRITE_BATCH_SIZE}"
            )
